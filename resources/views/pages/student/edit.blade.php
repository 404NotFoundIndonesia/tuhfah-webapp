        ...
        <form action="{{ route('student.update', $student) }}" method="post" enctype="multipart/form-data">
            <div class="card-header">
                <div class="d-flex align-items-start align-items-sm-center gap-4">
                    <a data-fslightbox href="{{ $student->image_url }}">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    <x-forms.input name="student_id_number" :value="$student->student_id_number"/>
                </div>
                <div class="mb-3">
                    <x-forms.input name="name" required :value="$student->name"/>
                </div>
                <div class="mb-3">
                    <x-forms.input name="nickname" :value="$student->nickname"/>
                </div>
                <div class="mb-3">
                    <x-forms.input-select2 name="gender" required :value="$student->gender"
                                           :options="[ ['male', __('label.male')], ['female', __('label.female')] ]"/>
                </div>
                <div class="mb-3">
                    <x-forms.input name="birthplace" :value="$student->birthplace"/>
                </div>
                <div class="mb-3">
                    <x-forms.input name="birthdate" type="date" required :value="$student->birthdate" />
                </div>
                <div class="mb-3">
                    <x-forms.input-select2 name="student_guardian_id" :value="$student->student_guardian_id" :options="array_map(fn($guardian) => [$guardian['id'], $guardian['name']], $guardians->toArray())" />
                </div>
                <div class="mb-3">
                    <x-forms.input-select2 name="status" required :value="$student->status"
                ...
                <div class="mb-3">
                    <x-forms.input name="admission_date" type="date" required :value="$student->admission_date" />
                </div>
                <div class="mb-3" id="departure-date-wrapper">
                    <x-forms.input name="departure_date" required type="date" :value="$student->departure_date" />
                </div>
                <div class="mt-4">
