...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="py-3 mb-0">
            <a class="text-muted fw-light" href="{{ route('student-guardian.index') }}">{{ __('menu.student_guardian') }} /</a>
            {{ __('label.edit') }}
        </h4>
        <div>
            <a href="{{ route('student-guardian.index') }}" class="btn btn-secondary text-white fw-medium">
                {{ __('button.back') }}
            </a>
        </div>
    </div>
    ...
    <div class="card mb-4">
        <form action="{{ route('student-guardian.update', $guardian) }}" method="post" enctype="multipart/form-data">
            <div class="card-header">
                <div class="d-flex align-items-start align-items-sm-center gap-4">
                    <a data-fslightbox href="{{ $guardian->image_url }}">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    <x-forms.input name="name" required :value="$guardian->name"/>
                </div>
                <div class="mb-3">
                    <x-forms.input name="email" type="email" required :value="$guardian->email"/>
                </div>
                <div class="mb-3">
                    <x-forms.input-select2 name="gender" required :value="$guardian->gender"
                                           :options="[ ['male', __('label.male')], ['female', __('label.female')] ]"/>
                </div>
                <div class="mb-3">
                    <x-forms.input name="phone" type="phone" required :value="$guardian->phone"/>
                </div>
                <div class="mb-3">
                    <x-forms.input-textarea name="address" :value="$guardian->address"/>
                </div>
                <div class="mb-3">
                    <x-forms.input-select2 name="marital_status" :value="$guardian->marital_status"
